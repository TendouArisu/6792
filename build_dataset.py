...
def stage_b_flatten(raw_dir, out_csv, post_fall_frames=50,
                    recursive=False, include_prefall=False):
    """
    ...
    recursive=True     : scan ALL .npz files under raw_dir (includes augmented
                         data and any extra dataset sub-folders such as URFD/).
    recursive=False    : original behaviour — only LE2I_SCENES sub-dirs.
    include_prefall=True : extend fall labels to cover the falling motion itself
                         (from `start` frame, not just `end`). Gives the model
                         earlier signal and removes mislabeled "falling=normal"
                         frames. Increases fall frames by ~45%.
    """
    ...
    if recursive:
        npz_files = sorted(glob.glob(
            os.path.join(raw_dir, '**', '*.npz'), recursive=True))
    else:
        npz_files = []
        for scene in LE2I_SCENES:
    ...
    print("          include_prefall  = {}".format(include_prefall))
    ...
    with open(out_csv, 'w', newline='') as f:
        ...
        for npz_path in npz_files:
            ...
            if start > 0 and end > 0 and not malformed:
                if include_prefall:
                    # Label from fall START through post-fall window.
                    # This captures the falling motion itself (removes the
                    # "falling person = normal" mislabeling) and enables
                    # earlier detection.
...
def parse_args():
    ...
    p.add_argument('--recursive', action='store_true',
                   help="Stage B: scan ALL .npz files under raw_keypoints/ "
                        "recursively (picks up augmented + extra datasets). "
                        "Default: only LE2I_SCENES sub-dirs.")
    p.add_argument('--include-prefall', action='store_true',
                   help="Stage B: extend fall labels to cover the falling "
                        "motion itself (from 'start' frame, not just 'end'). "
                        "Removes mislabeled falling-motion frames and enables "
                        "earlier detection. Increases fall frames by ~45%%.")
    return p.parse_args()
...
def main():
    ...
    if args.skip_flatten:
        ...
        stage_b_flatten(raw_dir, out_csv,
                        ...
                        include_prefall=args.include_prefall)

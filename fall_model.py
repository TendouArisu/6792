...
class FallDetectionGRU(nn.Module):
    """
    GRU fall-detection classifier.  Supports unidirectional and bidirectional.

    Input  : (B, T, input_dim)
    Output : (B, 2)   logits for [normal, fall]

    bidirectional=True processes each window in both temporal directions.
    At inference time the full 30-frame window is available, so the backward
    pass sees post-fall stillness while processing early fall frames — a
    natural fit for fixed-window fall detection.
    """

    def __init__(self, input_dim=FEATURE_DIM, hidden_dim=64, num_layers=2,
                 num_classes=2, dropout=0.3, input_dropout=0.0,
                 bidirectional=False):
        ...
        # Bidirectional doubles the last hidden state dimension
        clf_in = hidden_dim * 2 if bidirectional else hidden_dim
        self.classifier = nn.Sequential(
            nn.Linear(clf_in, max(clf_in // 2, 32)),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(max(clf_in // 2, 32), num_classes),
        )
    ...
    def load_from(cls, path, map_location='cpu'):
        ...
        cfg  = ckpt['config']
        if 'input_dropout' not in cfg:
            cfg['input_dropout'] = 0.0
        if 'bidirectional' not in cfg:
...
class FallKeypointCSVDataset(Dataset):
    ...
    def __init__(self, csv_path, seq_len=30, stride=5,
        ...
        smooth_alpha   : EMA smoothing factor for keypoint coords (0 disables)
    ...
    def _load(self, video_ids, verbose):
        ...
        ext = KeypointFeatureExtractor(smooth_alpha=self.smooth_alpha)
...
# =====================================================================
_AUG_SUFFIXES = ('_flip', '_fast', '_slow', '_rev')


def _base_video_id(vid: str) -> str:
    """Strip augmentation suffix to recover the original video ID."""
